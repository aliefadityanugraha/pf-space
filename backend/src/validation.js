...
const uploadOrUrl = z
  ...
  .trim()
  .refine(
    (v) => v === '' || isUrl(v) || isSafeUploadPath(v),
    'Invalid URL or upload path'
  );

const SAFE_UPLOAD_DIRS = ['videos', 'images', 'documents', 'avatars'];

/**
 * Accepts only local upload paths like "/uploads/<subfolder>/<filename>".
 * Rejects "..", absolute paths outside /uploads, and unknown subfolders.
 */
const isSafeUploadPath = (v) => {
  if (!v.startsWith('/uploads/')) return false;
  const relative = v.slice('/uploads/'.length);
  if (!relative || relative.includes('..') || relative.startsWith('/')) return false;
  const parts = relative.split('/');
  if (parts.length < 2) return false;
  const [subfolder, ...rest] = parts;
  if (!SAFE_UPLOAD_DIRS.includes(subfolder)) return false;
  // A single filename (with extension) is expected; nothing nested
  if (rest.length > 1 || !/^[\w.\-]+$/.test(rest.join('.'))) return false;
  return true;
};
...
export const filmCreateSchema = z.object({
  ...
  gambar_poster: uploadOrUrl.optional().or(z.literal('')),
  banner_url: uploadOrUrl.optional().or(z.literal('')),
  deskripsi_lengkap: z.string().optional(),
  file_naskah: uploadOrUrl.optional().or(z.literal('')),
  file_storyboard: uploadOrUrl.optional().or(z.literal('')),
  file_rab: uploadOrUrl.optional().or(z.literal('')),
  crew: z.any().optional(),
});
