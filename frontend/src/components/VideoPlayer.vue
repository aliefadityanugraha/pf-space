...
const handleDoubleClick = (e) => {
  ...
  if (x < rect.width / 2) {
    ...
    plyr.currentTime = Math.min(
      plyr.duration || plyr.currentTime + 10,
      plyr.currentTime + 10,
    );
...
<template>
  ...
  >
    ...
    <div
      v-if="failed && !isYoutubeUrl"
      class="absolute inset-0 flex items-center justify-center text-white/40 text-xs uppercase font-black tracking-widest bg-stone-900"
    >
      Gagal memuat video
    </div>
    ...
    >
      <div
        class="absolute bottom-6 right-8 text-white/30 font-mono font-bold text-[10px] md:text-sm tracking-widest text-shadow-sm whitespace-nowrap"
      >
    ...
    <transition name="seek-fade">
      <div
        v-if="seekIndicator === 'backward'"
        class="absolute left-[15%] top-1/2 -translate-y-1/2 flex flex-col items-center justify-center bg-black/40 text-white w-16 h-16 rounded-full pointer-events-none z-[100]"
      >
        <svg
    ...
    <transition name="seek-fade">
      <div
        v-if="seekIndicator === 'forward'"
        class="absolute right-[15%] top-1/2 -translate-y-1/2 flex flex-col items-center justify-center bg-black/40 text-white w-16 h-16 rounded-full pointer-events-none z-[100]"
      >
        <svg
          xmlns="http://www.w3.org/2000/svg"
          width="24"
