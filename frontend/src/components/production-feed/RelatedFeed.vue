...
<template>
  <section v-if="related.length || isLoading || isError" class="mt-14 md:mt-20">
    <div class="flex items-end justify-between gap-4 mb-6">
      <h2 class="font-display text-xl md:text-2xl font-bold text-stone-900 tracking-tight">
        Bacaan Terkait
      </h2>
      <router-link
        to="/feed"
    ...
    <div
      v-if="isError"
      class="flex flex-col items-center gap-3 border-2 border-black shadow-brutal-sm bg-white p-6 text-center"
      role="alert"
    >
      ...
      <button
        type="button"
        class="inline-flex items-center gap-1.5 border-2 border-black bg-brand-teal text-white font-body text-xs font-bold uppercase tracking-wide px-3 py-1.5 shadow-brutal-xs hover:translate-x-[1px] hover:translate-y-[1px] hover:shadow-none transition-all"
        @click="loadRelated"
      >
    ...
    <div v-else-if="isLoading" class="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-4">
      <div v-for="n in Math.min(limit, 4)" :key="n" class="border-2 border-stone-200 bg-white p-3">
    ...
    <div v-else class="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-4">
      <router-link
        ...
        class="group block border-2 border-black bg-white shadow-brutal-xs hover:shadow-brutal-md hover:-translate-x-[2px] hover:-translate-y-[2px] transition-all"
      >
        <div class="aspect-video w-full overflow-hidden bg-stone-200 border-b-2 border-black">
          <LazyImage
            :src="cover(post)"
            :alt="post.judul"
        ...
        <div class="p-3 md:p-4">
          <h3
            class="font-display font-bold text-sm md:text-base text-stone-900 leading-snug line-clamp-2 group-hover:text-brand-teal transition-colors"
          >
            {{ post.judul }}
          </h3>
          <p class="mt-2 text-[11px] md:text-xs font-mono text-stone-400 uppercase tracking-wider">
            {{ post.creator?.name || 'Creator' }}
          </p>
          <p
            v-if="post.publishedAt || post.createdAt"
            class="text-[10px] md:text-[11px] text-stone-400 font-body"
          >
