...
useHead({
  ...
  meta: [
    {
      name: "description",
      content: "Lihat dokumen naskah, storyboard, dan RAB arsip di PF Space.",
    },
  ],
});
...
const fetchFilm = async () => {
  ...
  try {
    ...
    // Verify asset exists
    if (
      !currentAssetConfig.value ||
      !film.value[currentAssetConfig.value.field]
    ) {
...
const goBack = () => {
  router.push({
    name: "ArchiveDetail",
    params: { slug: film.value?.slug || archiveSlug.value },
  });
};
  ...
  <PageLayout>
    <div class="max-w-7xl mx-auto px-4 md:px-8">
      ...
      <template v-else>
        <!-- Breadcrumb -->
        <nav
          class="flex items-center gap-1.5 md:gap-2 text-[10px] md:text-sm mb-6 flex-wrap"
        >
          <template v-for="(crumb, index) in breadcrumbs">
            ...
            <span
              :key="`span-${index}`"
              v-else
              class="font-bold uppercase tracking-wide text-stone-800 border-b-2 border-orange-400 pb-0.5 whitespace-nowrap"
            >
              {{ crumb.label }}
            </span>
            <span
        ...
        <!-- Toolbar -->
        <div
          class="bg-stone-200 rounded-xl border-2 border-stone-800 shadow-[4px_4px_0px_0px_rgba(43,38,38,1)] p-3 md:p-4 mb-6 transition-all"
        >
          <div
            class="flex flex-wrap items-center justify-between gap-3 md:gap-4"
          >
            <!-- Back Button & Title -->
            <div class="flex items-center gap-2.5 md:gap-4">
              ...
              <div class="flex items-center gap-2 md:gap-3">
                <div
                  class="w-9 h-8 md:w-11 md:h-10 bg-stone-100 rounded-lg border border-stone-300 flex items-center justify-center"
                >
                  <FileText class="w-4 h-4 md:w-5 md:h-5 text-teal-600" />
                </div>
                <div class="min-w-0">
                  <h1
                    class="text-sm md:text-xl font-serif text-stone-800 leading-tight truncate max-w-[150px] md:max-w-none"
                  >
                    {{ currentAssetConfig?.title }}
                  </h1>
                  <p
                    class="text-[9px] md:text-xs font-mono uppercase text-stone-600"
                  >
                    {{ currentAssetConfig?.type }} • PDF
                  </p>
                </div>
              </div>
            </div>
        ...
        <div
          class="bg-stone-700 rounded-xl border-2 border-stone-800 shadow-inner overflow-hidden min-h-[600px] lg:min-h-[800px] relative"
        >
          <iframe
