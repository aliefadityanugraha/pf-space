...
<template>
  ...
  >
    ...
    <div v-if="submitted" class="text-center py-8">
      <div
        class="w-16 h-16 bg-brand-teal/20 mx-auto mb-6 border-2 border-brand-teal shadow-brutal flex items-center justify-center"
      >
        <Send class="w-8 h-8 text-brand-teal" />
      </div>
      <h2
        class="text-xl font-display font-bold text-white mb-2 uppercase tracking-tighter"
      >
        Periksa Email Kamu
      </h2>
      ...
      <router-link to="/auth/login">
        <Button
          class="bg-stone-700 border-2 border-stone-600 text-white hover:bg-stone-600 w-full font-bold uppercase tracking-wider text-xs h-12"
        >
    ...
    <form v-else @submit.prevent="handleSubmit" class="space-y-6">
      ...
      <div>
        <label class="block text-white font-body mb-2 text-sm"
          >Alamat Email</label
        >
        <div class="relative">
          <Mail
    ...
    <template #footer>
      <div v-if="!submitted">
        <router-link
          to="/auth/login"
          class="text-sm text-amber-500 font-body hover:text-amber-400 inline-flex items-center gap-2 font-bold uppercase tracking-widest text-[10px]"
        >
