...
<template>
  ...
  >
    <AuthCard title="Reset Password" subtitle="Buat password baru anda">
      <div v-if="success" class="text-center py-8">
        ...
        <router-link to="/auth/login">
          <Button
            variant="outline"
            class="bg-stone-900 border-stone-700 text-white hover:bg-stone-800"
          >
            Login Sekarang
          </Button>
        </router-link>
      ...
      <form v-else @submit.prevent="handleSubmit" class="space-y-6">
        <div
          v-if="error"
          class="p-3 bg-red-500/10 border border-red-500/20 rounded text-red-400 text-sm text-center"
        >
          {{ error }}
        </div>
      ...
      <template #footer>
        <div v-if="!success">
          <router-link
            to="/auth/login"
            class="text-sm text-amber-500 font-body hover:text-amber-400 inline-flex items-center gap-2"
          >
