...
const saveMaterial = async () => {
  ...
  try {
    ...
    if (editingMaterial.value) {
      await api.put(
        `/api/learning-materials/${editingMaterial.value.materi_id}`,
        payload,
      );
...
const executeDelete = async () => {
  ...
  try {
    await api.delete(
      `/api/learning-materials/${materialToDelete.value.materi_id}`,
    );
      ...
      >
        ...
        <div class="min-w-0">
          ...
          >
            ...
            <li>
              Pastikan judul materi deskriptif dan mudah dicari oleh siswa.
            </li>
            <li>
              Video harus menggunakan link YouTube yang valid (Full atau
              Shortened).
            </li>
            <li>
              Gunakan status "Aktif" untuk menampilkan materi di halaman publik
              secara instan.
            </li>
          </ul>
        </div>
      ...
      >
        ...
        <CardContent class="p-0 overflow-x-auto bg-white dark:bg-stone-900">
          ...
          >
            ...
            <span
              class="font-mono text-xs uppercase tracking-widest text-stone-500"
              >Memuat data...</span
            >
          </div>
                ...
                >
                  <td class="px-6 py-4">
                    ...
                    <div class="flex items-center gap-1.5 mt-1">
                      <span
                        class="text-[9px] font-black uppercase text-brand-teal"
                        >{{ m.kategori || "Dasar Perfilman" }}</span
                      >
                      <span class="text-stone-300">|</span>
                      <div
                        class="text-[10px] text-stone-500 line-clamp-1 truncate max-w-[200px]"
                      >
                        {{ m.deskripsi || "-" }}
                      </div>
                    </div>
                  ...
                  <td class="px-6 py-4">
                    <Badge
                      v-if="m.tipe === 'pdf'"
                      variant="outline"
                      class="bg-red-50 text-red-700 border-red-200 uppercase text-[10px]"
                    >
                    ...
                    <Badge
                      v-else
                      variant="outline"
                      class="bg-blue-50 text-blue-700 border-blue-200 uppercase text-[10px]"
                    >
                  ...
                  <td class="px-6 py-4">
                    <div class="flex items-center gap-2">
                      <div
                        class="w-6 h-6 rounded-full bg-stone-200 border border-black overflow-hidden flex-shrink-0"
                      >
                        <img
                          v-if="m.creator?.image"
                          :src="m.creator.image"
                  ...
                  <td class="px-6 py-4">
                    ...
                    >
                      <div
                        :class="[
                          m.is_active ? 'bg-green-500' : 'bg-stone-300',
                          'w-10 h-5 border-2 border-black rounded-full relative transition-colors',
                          togglingIds.has(m.materi_id) ? 'opacity-50' : '',
                        ]"
                      >
                        <div
                          :class="[
                            m.is_active ? 'translate-x-5' : 'translate-x-0',
                            'absolute top-0.5 left-0.5 w-3 h-3 bg-white border border-black rounded-full transition-transform',
                          ]"
                        ></div>
                      </div>
                      <span
                        :class="[
                          m.is_active
                            ...
                            : 'text-stone-400',
                          'text-[10px] uppercase tracking-tighter flex items-center gap-1',
                        ]"
                      >
                        <Loader2
                          v-if="togglingIds.has(m.materi_id)"
                  ...
                  <td class="px-6 py-4 text-right">
                    <div class="flex gap-2 justify-end">
                      <Button
                        ...
                        size="sm"
                        @click="openModal(m)"
                        class="border-2 border-black p-2 h-8"
                      >
                      ...
                      <Button
                        ...
                        class="border-2 border-black p-2 h-8 text-red-600 hover:bg-red-50"
                      >
      ...
      >
        ...
        >
          ...
          <div class="flex gap-3 pt-4">
            ...
            <Button
              type="submit"
              class="flex-1 gap-2 border-2 border-black shadow-brutal-sm"
    ...
    >
      ...
      >
        ...
        <div class="flex gap-3">
          ...
          >
          <Button
            class="flex-1 bg-red-600 hover:bg-red-700 text-white border-2 border-black gap-2"
            @click="executeDelete"
            :disabled="deleting"
          >
