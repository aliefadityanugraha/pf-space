...
const isOwner = computed(
  () => film.value && user.value && film.value.user_id === user.value.id,
);
...
const criteria = [
  {
    id: "script",
    label: "Naskah & Cerita",
    scoreKey: "script_score",
    commentKey: "script_comment",
  },
  {
    id: "cinematography",
    label: "Teknis & Sinematografi",
    scoreKey: "cinematography_score",
    commentKey: "cinematography_comment",
  },
  {
    id: "editing",
    label: "Editing & Ritme",
    scoreKey: "editing_score",
    commentKey: "editing_comment",
  },
  {
    id: "production",
    label: "Produksi & Dokumen",
    scoreKey: "production_score",
    commentKey: "production_comment",
  },
];
...
const addScene = async () => {
  ...
  scenes.value = [...scenes.value, scene].sort(
...
const syncScenes = async () => {
  ...
  try {
    const res = await api.post(
      `/api/film-scenes/${film.value.film_id}`,
    ...
    const msg =
      err.data?.message || err.message || "Gagal menyimpan struktur adegan";
    showToast(msg, "error");
  } finally {
...
const saveEvaluation = async () => {
  ...
  try {
    const res = await api.post(
      `/api/evaluations/${film.value.film_id}`,
      evalForm,
    );
    evaluation.value = res.data;
    showToast(
      "Penilaian karya telah disimpan dan dikirim ke kreator.",
      "success",
    );
  } catch (err) {
...
const activeDocUrl = computed(() => {
  ...
  switch (activeTab.value) {
    case "naskah":
      return film.value.file_naskah;
    case "storyboard":
      return film.value.file_storyboard;
    case "rab":
      return film.value.file_rab;
    default:
      return null;
  }
});
...
useHead({
  title: computed(() =>
    film.value ? `Studi: ${film.value.judul} - PF Space` : "Loading...",
  ),
});
</script>
  ...
  >
        ...
        <div>
          ...
          <div class="space-y-2">
            ...
            >
              ...
              <span class="flex-1 text-left">Naskah</span>
            </button>
            <button
            ...
            >
              ...
              <span class="flex-1 text-left">Storyboard</span>
            </button>
            <button
              v-if="film?.file_rab"
              @click="selectTab('rab')"
            ...
            >
              ...
              <span class="flex-1 text-left">RAB</span>
            </button>
            <button
            ...
            >
              ...
              <span class="flex-1 text-left">Struktur Adegan</span>
            </button>
          </div>
        </div>
    ...
    >
          ...
          <template v-if="film.link_video_utama">
            ...
            >
              <div class="relative w-12 h-12 mb-4">
                ...
                ></div>
                <div
                  class="absolute inset-0 border-2 border-t-brand-teal rounded-full animate-spin"
                ></div>
              </div>
              <p
                class="text-[10px] font-black uppercase tracking-[0.2em] text-brand-teal animate-pulse"
              >
                Menyiapkan Pemutar...
              </p>
            </div>
      ...
      >
                ...
                >
                  <div class="flex items-center gap-3">
                    ...
                    <div class="flex flex-col">
                      ...
                      >
                      <span
                        class="text-xs font-bold text-brand-teal uppercase tracking-widest"
                        >{{ evaluation.moderator?.name || "Moderator" }}</span
                      >
                    </div>
                  </div>
                  ...
                  >
                    ...
                    >
                      {{ evaluation[item.scoreKey]
                        ...
                        class="text-[10px] text-stone-400 font-bold ml-0.5"
                        >/10</span
                      >
                    </div>
                    <h4
        ...
        >
          ...
          >
            <div class="flex items-center gap-3">
              ...
              <div>
                ...
                <p
                  class="text-[10px] text-stone-400 font-bold uppercase tracking-wider mt-1"
                >
                  Tautkan analisis Anda pada menit tertentu
                </p>
              </div>
            </div>
          </div>
          ...
          <!-- Notes List -->
          <div
            class="flex-1 overflow-y-auto p-4 md:p-6 space-y-4 no-scrollbar pb-10"
          >
            <div
              v-if="loadingNotes"
        ...
        <div v-else class="flex-1 w-full h-full bg-stone-950 relative">
          <template v-if="activeDocUrl">
            <iframe
              :src="
                activeDocUrl + '#view=FitH&scrollbar=0&toolbar=0&navpanes=0'
              "
    ...
    >
      <div class="relative w-16 h-16">
        <div
          class="absolute inset-0 border-4 border-white/10 rounded-full"
        ></div>
        <div
          class="absolute inset-0 border-4 border-t-brand-teal rounded-full animate-spin"
        ></div>
      </div>
      <p
        class="text-xs font-black uppercase tracking-[0.3em] text-stone-400 animate-pulse"
      >
        Menyiapkan Ruang Studi...
      </p>
    </div>
